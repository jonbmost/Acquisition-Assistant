...
This project deploys to **Vercel** from the `main` branch. The `vercel.json` file in the repo pins the Vite build settings, SPA routing, Node 20 runtime for API routes, and a reproducible `npm ci` install so production matches local behavior.
...
- **Build/install failures on Vercel**: ensure the project is using Node 20+ (package.json `engines.node`) and that Vercel is honoring the repo’s `npm ci` + `npm run build` commands. Re-run a failed build from the dashboard after confirming those settings.
