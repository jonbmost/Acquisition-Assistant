...
This project deploys to **Vercel** from the `main` branch. The `vercel.json` file in the repo pins the Vite build settings, SPA routing, **Node 24** for builds + API routes, and a reproducible `npm ci` install so production matches local behavior.
...
3. Confirm the project is configured to deploy from the `main` branch with **Install Command** `npm ci`, **Build Command** `npm run build`, and **Output Directory** `dist` (declared in `vercel.json`). The repository now includes a `.nvmrc` and `package.json` `engines.node` entry so Vercel picks Node 24 during both build and runtime. If the dashboard shows a different Node version, set it explicitly to **24.x**.
...
### Prerequisites
- Node.js 24 and npm (run `nvm use` in the repo root to auto-select the pinned Node version from `.nvmrc`)
