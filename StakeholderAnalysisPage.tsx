...
interface StakeholderAnalysisPageProps {
  currentRoute?: string;
...
const StakeholderAnalysisPage: React.FC<StakeholderAnalysisPageProps> = ({ currentRoute = '/stakeholder-analysis', onNavigate }) => {
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    if (!trimmed) {
      setError('Please describe the project or acquisition effort before submitting.');
      return;
    }
    ...
    try {
      ...
      setResult(text);
      setResultHtml(formatTextAsHtml(text, 'Stakeholder Analysis'));
    } catch (err) {
  ...
  const handleDownloadPdf = () => {
    ...
    try {
      ...
      doc.save('stakeholder-analysis.pdf');
    } finally {
      ...
      <main className="max-w-5xl mx-auto px-4 py-8">
        <div className="bg-gray-800/60 border border-gray-700 rounded-xl shadow-lg p-6 md:p-8">
          <div className="flex items-center justify-between mb-6 gap-4 flex-wrap">
            <div>
              <p className="text-sm uppercase tracking-wide text-cyan-400 font-semibold">Stakeholder Analysis</p>
              <h2 className="text-2xl font-bold mt-2">Generate a stakeholder analysis</h2>
              <p className="text-gray-300 mt-2 text-sm md:text-base">
                Describe the project or acquisition effort. The assistant will outline key stakeholders, roles, interests, and engagement strategies.
              </p>
            </div>
          </div>

          <form onSubmit={handleSubmit} className="space-y-4">
            <label className="block text-sm font-semibold text-gray-200">Project or acquisition description</label>
            <textarea
              ...
              placeholder="Example: Implementing an enterprise case management platform across multiple regional offices."
              value={description}
            ...
            <div className="flex items-center gap-3 flex-wrap">
              ...
              >
                {isLoading ? 'Requesting analysis…' : 'Generate stakeholder analysis'}
              </button>
              <p className="text-sm text-gray-400">Sends your description to the MCP-enabled stakeholder analysis endpoint.</p>
            </div>
          </form>

          <div className="mt-8">
            <h3 className="text-lg font-semibold text-gray-100 mb-3">Generated analysis</h3>
            <div className="min-h-[120px] rounded-lg border border-gray-700 bg-gray-900/60 p-4">
              ...
              {!error && !result && !isLoading && (
                <p className="text-gray-500">Your stakeholder analysis will appear here.</p>
              )}
              {isLoading && !error && (
                <p className="text-cyan-300 animate-pulse">Generating stakeholder analysis…</p>
              )}
            </div>
            <div className="mt-3 flex flex-wrap items-center gap-3">
              ...
              <p className="text-sm text-gray-400">Downloads enable once an analysis is generated.</p>
            </div>
          </div>
        </div>
      </main>
...
export default StakeholderAnalysisPage;
