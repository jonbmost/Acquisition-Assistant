...
const RequirementDocsPage: React.FC<RequirementDocsPageProps> = ({ currentRoute = '/requirement-docs', onNavigate }) => {
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    if (!trimmed) {
      setError('Please describe the requirement or mission need before submitting.');
      return;
    }
    ...
    try {
      ...
      setResult(text);
      setResultHtml(formatTextAsHtml(text, 'Requirements Document Draft'));
    } catch (err) {
      ...
      <main className="max-w-5xl mx-auto px-4 py-8">
        <div className="bg-gray-800/60 border border-gray-700 rounded-xl shadow-lg p-6 md:p-8">
          <div className="flex items-center justify-between mb-6 gap-4 flex-wrap">
            <div>
              <p className="text-sm uppercase tracking-wide text-cyan-400 font-semibold">Requirements workspace</p>
              <h2 className="text-2xl font-bold mt-2">Generate a requirements document</h2>
              <p className="text-gray-300 mt-2 text-sm md:text-base">
                Provide a short description of your requirement or mission need. The assistant will draft a requirements document you can refine.
              </p>
            </div>
          </div>

          <form onSubmit={handleSubmit} className="space-y-4">
            <label className="block text-sm font-semibold text-gray-200">Requirement or mission need</label>
            <textarea
              ...
              placeholder="Example: Develop and deploy a web-based portal to manage vendor onboarding and performance reporting across multiple agencies."
              value={description}
            ...
            <div className="flex items-center gap-3 flex-wrap">
              ...
              >
                {isLoading ? 'Requesting draft…' : 'Generate requirements draft'}
              </button>
              <p className="text-sm text-gray-400">The request is sent to the MCP-enabled requirements endpoint.</p>
            </div>
          </form>

          <div className="mt-8">
            <h3 className="text-lg font-semibold text-gray-100 mb-3">Generated document</h3>
            <div className="min-h-[120px] rounded-lg border border-gray-700 bg-gray-900/60 p-4">
              ...
              {!error && !result && !isLoading && (
                <p className="text-gray-500">Your generated requirements document will appear here.</p>
              )}
              {isLoading && !error && (
...
export default RequirementDocsPage;
